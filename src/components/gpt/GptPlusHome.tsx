      ...
      <div className="flex flex-col lg:flex-row justify-center gap-[24px] lg:gap-[10px]">
        {plans.map((plan, index) => (
          ...
          >
            ...
            <h3 className="font-medium text-[18px] font-poppins">{plan.name}</h3>
            <p className={`text-[#A1A6B4] text-[14px] font-poppins font-medium ${plan.highlight ? "text-[#C9C9C9]" : ""}`}>
              Display stars in Google organic search results and showcase
              reviews on your website
            </p>

            </div>
            <ul className={`text-left space-y-2 mb-4 text-[12px] font-medium font-Inter text-[#A1A6B4] ${plan.highlight ? "text-[#F0F2F5]" : ""}` }>
            ...
            <p className="text-[40px] font-Inter font-bold text-[#E5E7EB]">
              ${plan.price} <span className="text-[12px] block font-medium text-[#A1A6B4]">per month</span>
            </p>
            <button
